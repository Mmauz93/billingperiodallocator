...
// Import the correct type from InvoiceForm
import type { CalculationCallbackData } from "@/components/invoice-form";
import { CalculationResult } from "@/lib/calculations"; // Import CalculationResult
...
export default function Home() {
  ...
  // Store relevant input data when calculation is successful - type allows null
  const [storedInputData, setStoredInputData] =
    useState<CalculationCallbackData>(null); // Use CalculationCallbackData type for state

  // Updated handleCalculation signature to match InvoiceForm callback type
  const handleCalculation = (
    formData: CalculationCallbackData, // Use the type from InvoiceForm
    ...
    // Only store formData if it's not null and there are results without error
    if (formData && results && !error) {
      setStoredInputData(formData);
    } else {
      // Explicitly set to null otherwise
    ...
    <main className="flex flex-col items-center grow w-full p-4 md:p-12 lg:p-24">
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl grow">
        ...
        <h1 className="text-3xl font-bold text-center mb-8">
          Invoice Split Calculator
        </h1>
        <div className="w-full">
          {" "}
          {/* Form takes full width */}
          <InvoiceForm onCalculateAction={handleCalculation} />{" "}
          {/* Pass handler with correct prop name */}
        </div>
        ...
        <div id="results-section" className="mt-12 w-full flex justify-center">
          ...
          {calculationResult && storedInputData && !calculationError && (
            <ResultsDisplay
              ...
              // Cast storedInputData here if ResultsDisplay requires non-null, 
              // but the check above should ensure it's not null here.
              // If ResultsDisplay inputData prop type needs update, do that instead.
