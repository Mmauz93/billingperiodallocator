...
describe('calculateInvoiceSplit', () => {
    ...
    // Test spanning two years including a leap day (Feb 29th, 2024)
     test('should split correctly across two years including leap day (inclusive)', () => {
        ...
        expect((split2023?.totalSplitAmount || 0) + (split2024?.totalSplitAmount || 0)).toBeCloseTo(result.adjustedTotalAmount, 2);
    });
    ...
    test('should split single amount correctly within one year (aggregated)', () => {
        ...
        expect(result.resultsPerAmount[0].splits[2023].splitAmount).toBeCloseTo(1100, 2);
    });
    ...
    test('should return error if end date is before start date', () => {
        ...
        expect(result.calculationSteps?.error).toContain('Start date must be before');
    });
     ...
     test('should return error if no amounts provided', () => {
        ...
        expect(result.aggregatedSplits).toHaveLength(0);
        expect(result.resultsPerAmount).toHaveLength(0);
        expect(result.calculationSteps?.error).toContain('At least one amount is required');
    });
    ...
    test('should split multiple amounts correctly across two years (inclusive) - detailed check', () => {
        ...
        expect((agg2023?.totalSplitAmount || 0) + (agg2024?.totalSplitAmount || 0)).toBeCloseTo(inputTotal, 2);

        const breakdown1 = result.resultsPerAmount.find(r => r.originalAmount === 1000);
        expect(breakdown1).toBeDefined();
        expect(breakdown1?.adjustedTotalAmount).toBeCloseTo(1000, 2);
        expect(breakdown1?.splits[2023]?.splitAmount).toBeCloseTo(1000 * prop2023, 2);
        expect(breakdown1?.splits[2024]?.splitAmount).toBeCloseTo(1000 * prop2024, 2);
        expect((breakdown1?.splits[2023]?.splitAmount || 0) + (breakdown1?.splits[2024]?.splitAmount || 0)).toBeCloseTo(1000, 2);

        const breakdown2 = result.resultsPerAmount.find(r => r.originalAmount === 80);
        ...
        expect((breakdown2?.splits[2023]?.splitAmount || 0) + (breakdown2?.splits[2024]?.splitAmount || 0)).toBeCloseTo(80, 2);
    });
